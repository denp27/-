...
from app.helpers import (
    check_subscription, show_subscription_required, exit_button, unified_send,
    franchise_branding, get_section_photo, send_start_screen, get_start_media_path,
)
...
async def show_start(event, bot: Bot, franchise_id: int = 0):
    ...
    if isinstance(event, CallbackQuery):
        ...
        if message and (message.animation or message.video):
            try:
    ...
    if get_start_media_path():
        await send_start_screen(bot, chat_id, text, markup)
    else:
