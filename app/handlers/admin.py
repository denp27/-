...
from app.config import (
    ...
    COMMISSION_NFT, COMMISSION_NUMBERS, STARS_PRICE_MODE, STARS_PRICE_RUB
)
from app.helpers import format_stars_price
...
async def _show_adminka_text(bot: Bot, user_id: int):
    ...
    text = (
        ...
        f'<blockquote>'
        f'Stars: {format_stars_price()}  ·  TON: {COMMISSION_TON[0]}%  ·  Premium: {COMMISSION_PREMIUM[0]}%\n'
...
async def settings(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    # Наценки — 3 пары
    builder.button(text=f"Stars {format_stars_price()}", callback_data="change_commission_stars", icon_custom_emoji_id="5904462880941545555")
    ...
    await bot.edit_message_text(
        text=(
            ...
            f'├ Stars: <code>{format_stars_price()}</code>  ·  TON: <code>{COMMISSION_TON[0]}%</code>\n'
...
async def change_commission_stars(call: CallbackQuery, bot: Bot, state: FSMContext):
    user_id = call.from_user.id
    cost = await fragment.get_price_star()
    cost_line = f"{Decimal(str(cost)):.2f} ₽" if cost else "недоступна"
    await bot.edit_message_text(
        text=(
            f"<b>Изменение цены Stars</b>\n\n"
            f"<b>Закупка сейчас:</b> {cost_line}\n"
            f"<b>Текущая цена:</b> {format_stars_price()}\n\n"
            f"<b>Введите новую цену:</b>\n"
            f"├ <code>1.3</code> — фикс за 1 звезду в ₽\n"
            f"└ <code>15%</code> — наценка в процентах"
        ),
        chat_id=user_id, message_id=call.message.message_id, parse_mode="HTML",
        reply_markup=InlineKeyboardBuilder().button(text="Отмена", callback_data="settings", icon_custom_emoji_id="5870657884844462243").as_markup()
    )
    await state.set_state(AdminState.wait_stars_price)


@router.message(AdminState.wait_stars_price)
async def process_stars_price(message: Message, bot: Bot, state: FSMContext):
    user_id = message.from_user.id
    raw = (message.text or "").strip().lower()
    raw = raw.replace(" ", "").replace("₽", "").replace("руб.", "").replace("руб", "").replace("р.", "")
    ...
    try:
        ...
        await bot.send_message(
            user_id,
            "❌ Введите число, например <code>1.3</code> или <code>15%</code>",
            parse_mode="HTML",
            reply_markup=await admin_exit_button(),
        )
        return
    ...
    if is_percent:
        if value < 0 or value > 100:
            ...
            return
        STARS_PRICE_MODE[0] = "percent"
        COMMISSION_STARS[0] = int(value)
        label = f"{int(value)}%"
        warning = ""
        env_updates = {
            "STARS_PRICE_MODE": "percent",
            "COMMISSION_STARS": str(int(value)),
        }
    else:
        if value <= 0 or value > 20:
            await bot.send_message(user_id, "❌ Цена за звезду должна быть от 0.01 до 20 ₽", reply_markup=await admin_exit_button())
            return
        rounded = round(value, 2)
        STARS_PRICE_MODE[0] = "fixed"
        STARS_PRICE_RUB[0] = rounded
        label = f"{Decimal(str(rounded)).quantize(Decimal('0.01'))} ₽"
        cost = await fragment.get_price_star()
        warning = ""
        if cost and Decimal(str(rounded)) < Decimal(str(cost)):
            warning = (
                f"\n\n⚠️ Цена ниже закупки ({Decimal(str(cost)):.2f} ₽). "
                f"В продаже будет себестоимость, пока курс не упадёт."
            )
        env_updates = {
            "STARS_PRICE_MODE": "fixed",
            "STARS_PRICE_RUB": str(rounded),
        }

    try:
        from dotenv import set_key
        for key, val in env_updates.items():
            set_key(str(config.ENV_FILE), key, val)
    except Exception as e:
        print(f"Warning: could not update .env: {e}")

    await bot.send_message(
        user_id,
        f"✅ <b>Цена Stars обновлена</b>\n\n🆕 Теперь: <b>{label}</b>{warning}",
        parse_mode="HTML",
        reply_markup=await admin_exit_button(),
    )
    await state.clear()
...
async def process_new_commission(message: Message, bot: Bot, state: FSMContext):
    ...
    try:
        ...
        if commission_type == "COMMISSION_TON":
