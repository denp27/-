...
from app.helpers import exit_button, ebal_button, calc_star_sell_price, get_section_photo, unified_send, send_admin_notification, process_franchise_reward
from app.states import UserState
from app.config import AVAILABLE_URL
...
async def buy_stars_self(call: CallbackQuery, bot: Bot, state: FSMContext, franchise_id: int = 0):
    ...
    star_price = await calc_star_sell_price(price, franchise_id)
...
async def _execute_stars_purchase(bot: Bot, user_id: int, username: str, stars: int, star_price: Decimal, price: Decimal, message_id: int = None, franchise_id: int = 0) -> bool:
    ...
    try:
        ...
        print(f"[stars] Ошибка отправки звёзд для user={user_id}: {e}")
...
async def quick_stars_buy(call: CallbackQuery, bot: Bot, state: FSMContext, franchise_id: int = 0):
    ...
    async def purchase_task():
        return await _execute_stars_purchase(bot, user_id, username, stars, star_price, price, message_id, franchise_id)
...
async def buy_stars_stars(message: Message, bot: Bot, state: FSMContext, franchise_id: int = 0):
    ...
    async def purchase_task():
        return await _execute_stars_purchase(bot, user_id, username, stars, star_price, price, queued_msg.message_id, franchise_id)
